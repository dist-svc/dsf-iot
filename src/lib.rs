...
impl IotClient {
    ...
    pub async fn search(&mut self, id: &Id) -> Result<(ServiceHandle, LocateInfo), ClientError> {
        self.client.locate(LocateOptions{ id: id.clone(), local_only: false }).await
    }
    ...
    pub async fn register(&mut self, options: RegisterOptions) -> Result<dsf_rpc::service::RegisterInfo, ClientError> {
         self.client.register(options).await
    }

    /// Publish an existing IoT service
    pub async fn publish_raw(&mut self, service: ServiceIdentifier, kind: DataKind, data: &[u8]) -> Result<PublishInfo, ClientError> {
    ...
    pub async fn subscribe(&mut self, options: rpc::SubscribeOptions) -> Result<impl Stream<Item=()>, ClientError> {
